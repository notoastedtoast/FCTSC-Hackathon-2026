...
class FrontendTests(unittest.TestCase):
    ...
    def test_static_page_calls_backend_and_keeps_results_separate(self) -> None:
        ...
        self.assertIn('href="./styles.css?v=42"', page)
        self.assertIn('src="./html2canvas.min.js"', page)
        self.assertIn('src="./app-data.js"', page)
        self.assertIn('src="./app-render.js?v=42"', page)
        self.assertIn('src="./app.js?v=42"', page)
        ...
        self.assertLess(
            ...
            page.index('src="./app-render.js?v=42"'),
        )
        ...
        self.assertIn("originalMessage.querySelectorAll('mark')", script)
        self.assertIn("node.className='capture-highlighted-excerpt'", script)
        self.assertIn("originalMessage.replaceChildren(...excerptNodes)", script)
        ...
        self.assertIn("windowWidth:680", script)
        self.assertIn("scale:2", script)
        ...
        self.assertIn("function renderPsychology(originalText,payload,{allowResponderQuestion=true}={})", script)
        self.assertIn("postAnalysisQuestion.dataset.eligible=String(Boolean(allowResponderQuestion&&shouldShow&&!payload.offline&&payload.id&&shouldOfferResponder))", script)
        self.assertIn("renderPsychology(text,payload,{allowResponderQuestion:!fromHistory})", script)
        ...
        self.assertIn("name!=='Phản ánh tin nhắn/cuộc gọi rác, lừa đảo'", script)
        self.assertIn("name==='Công an'||name==='Phản ánh tin nhắn/cuộc gọi rác, lừa đảo'", script)
        ...
        self.assertIn("width:680px", styles)
        self.assertIn(".result-image-capture.result-section{width:652px", styles)
        self.assertIn(".result-image-capture-footer{width:652px", styles)
        self.assertIn(".result-image-capture .detective-message-row{grid-template-columns:36px minmax(0,1fr);gap:8px;align-items:start", styles)
        self.assertIn(".result-image-capture .detective-message-bubble::before{top:14px;bottom:auto}", styles)
        self.assertIn("line-height:1.7;overflow-wrap:anywhere", styles)
        self.assertIn(".result-image-capture .original-message.capture-highlighted-message", styles)
        self.assertIn(".result-image-capture .capture-highlighted-excerpt{display:block", styles)
        ...
        self.assertIn('CACHE_NAME="scamcheck-shell-v42"', service_worker)
        self.assertIn(
