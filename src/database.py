...
class _SqliteTestRepository:
    ...
    async def get_history(self, session_id: str) -> list[HistoryEntry]:
        if self._is_in_memory:
            return self._get_history_sync(session_id)
    ...
    async def hide_history(self, session_id: str, record_id: str) -> bool:
        if self._is_in_memory:
            return self._hide_history_sync(session_id, record_id)
        return await asyncio.to_thread(self._hide_history_sync, session_id, record_id)
    ...
    def _get_history_sync(self, session_id: str) -> list[HistoryEntry]:
        try:
            with self._connection() as connection, connection:
                self._create_schema(connection)
                rows = connection.execute(
                    """
                    SELECT ar.response_json, a.message_text, a.created_at
                    FROM analysis_requests AS ar
                    JOIN analyses AS a
                      ON a.id = json_extract(ar.response_json, '$.id')
                    WHERE ar.session_id = ? AND ar.status = 'completed'
                      AND ar.history_hidden = 0
            ...
            raise DatabaseError("Unable to retrieve analysis history") from exc
        try:
            entries: list[HistoryEntry] = []
            for response_json, message, created_at in rows:
                response = AnalyzeResponse.model_validate_json(response_json)
                entries.append(
                    HistoryEntry(
                        id=response.id,
                        message=message,
            ...
            raise DatabaseError("Stored analysis history is invalid") from exc

    def _hide_history_sync(self, session_id: str, record_id: str) -> bool:
        try:
            with self._connection() as connection, connection:
                self._create_schema(connection)
                rows = connection.execute(
                ...
                request_id = next(
                    (
                        row[0]
                        for row in rows
                        if AnalyzeResponse.model_validate_json(row[1]).id == record_id
                    ),
                    None,
                )
                ...
                cursor = connection.execute(
                    """
                    UPDATE analysis_requests SET history_hidden = 1
                    WHERE session_id = ? AND request_id = ?
                    """,
                    (session_id, request_id),
                )
            ...
            raise DatabaseError("Unable to hide analysis history") from exc
    ...
    def _create_schema(connection: sqlite3.Connection) -> None:
        ...
        if "history_hidden" not in request_columns:
            connection.execute(
                "ALTER TABLE analysis_requests ADD COLUMN history_hidden "
...
class _PostgresRepository:
    ...
    def _get_history_sync(self, session_id: str) -> list[HistoryEntry]:
        try:
            with self._connect() as connection:
                ...
                rows = connection.execute(
                    ...
                    SELECT ar.response_json, a.message_text, a.created_at
                    FROM analysis_requests AS ar
                    JOIN analyses AS a ON a.id = (ar.response_json ->> 'id')
                    WHERE ar.session_id = %s AND ar.status = 'completed'
                      AND NOT ar.history_hidden
                    ORDER BY ar.updated_at DESC
                    LIMIT 10
                    """,
                    (session_id,),
        ...
        try:
            entries: list[HistoryEntry] = []
            for response_json, message, created_at in rows:
                response = AnalyzeResponse.model_validate(response_json)
                entries.append(
                    HistoryEntry(
                        id=response.id,
                        message=message,
    ...
    def _hide_history_sync(self, session_id: str, record_id: str) -> bool:
        try:
            with self._connect() as connection:
                ...
                cursor = connection.execute(
                    """
                    UPDATE analysis_requests SET history_hidden = TRUE
                    WHERE session_id = %s AND status = 'completed'
                      AND NOT history_hidden AND (response_json ->> 'id') = %s
                    """,
    ...
    def _create_schema(connection: Connection[Any]) -> None:
        ...
        connection.execute(
            "ALTER TABLE analysis_requests ADD COLUMN IF NOT EXISTS "
            "history_hidden BOOLEAN NOT NULL DEFAULT FALSE"
        )
...
class AnalysisRepository:
    ...
    async def hide_history(self, session_id: str, record_id: str) -> bool:
        return await self._backend.hide_history(session_id, record_id)
