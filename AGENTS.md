...
- `src/main.py`: FastAPI composition, lifespan, dependency protocols, cookie middleware,
  ...
  and the seven explicit frontend asset routes. Keep provider logic out of routes and PostgreSQL
...
- `frontend/app.js`: `/analyze` integration, AI-call `used`/`limit` display and limit-state
  handling, safe result rendering, voice input, browser-local recent-message history, the
  API-backed scam library, and local recognition prompts/grading/score. It keeps the
