...
gian chờ tối đa 12 giây và lượt Cô tâm lý tối đa 7,5 giây. Chuỗi dự phòng phân bổ lại phần
thời gian chưa dùng cho model tiếp theo; Groq dùng JSON Schema nghiêm ngặt và ngân sách đầu
ra tối thiểu để tránh trả về JSON dở dang. Khi hết lượt, backend trả thông
...
3. Thêm `GEMINI_API_KEY`. Cấu hình trong `.env.example` dùng `gemini-3.5-flash`, sau đó
   `gemini-3.1-flash-lite`. Có thể đổi bằng `GEMINI_MODEL` và
